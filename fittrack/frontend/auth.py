...
def render_login_form() -> None:
    ...
    if not username_or_email or not password:
        st.error("Enter both a username/email and a password.")
        return

    try:
        token_data = api_client.login(username_or_email, password)
        token = token_data["access_token"]
        user = api_client.get_current_user(token)
        state.log_in(token, user)
        st.rerun()
    except httpx.HTTPStatusError as exc:
        st.error(_extract_detail(exc) or "Login failed. Check your credentials.")
...
def render_register_form() -> None:
    ...
    try:
        ...
        st.error(_extract_detail(exc) or "Registration failed.")
    except httpx.RequestError:
        st.error("Could not reach the backend. Is FastAPI running?")
...
def _extract_detail(exc: httpx.HTTPStatusError) -> Optional[str]:
    try:
