...
if health is not None and health.get("status") == "ok":
    ...
    st.caption(
        ...
        f"{health.get('application', 'FitTrack')}"
    )
...
if not state.is_authenticated():
    ...
    with login_tab:
        ...
        st.caption(
            ...
            "trainer@fittrack.local · "
            "member@fittrack.local "
            "(see README for the password)."
        )
...
auth.render_change_password_form()
...
if role == "ADMIN":
    ...
    st.caption(
        "Full admin dashboard is built out in later steps."
    )
    ...
    st.caption(
        "Trainer dashboard is built out in later steps."
    )
    ...
    st.caption(
        "Member dashboard is built out in later steps."
    )