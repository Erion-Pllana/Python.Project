...
def test_register_creates_member_account():
    ...
    assert body["member_profile"]["first_name"] == "Alice"
...
def test_register_rejects_duplicate_username():
    _register()
    assert _register(email="someone-else@example.com").status_code == 409


def test_register_rejects_duplicate_email():
    _register()
    assert _register(username="someone_else").status_code == 409
...
def test_login_with_wrong_password_is_rejected():
    _register()
    response = client.post("/api/auth/login", json={
        "username_or_email": "alice", "password": "the-wrong-password",
    })
...
def test_login_with_unknown_user_is_rejected():
    response = client.post("/api/auth/login", json={
        "username_or_email": "nobody-registered", "password": "whatever123",
    })
    assert response.status_code == 401
